...
=end
						...
						rank_rates = {}
						game_type1_range_stats.keys.sort.each do |type1_id|
							game_type1_range_stats[type1_id].keys.sort.each do |rank_range|
								rank_rates[rank_range] ||= []
								rank_rates[rank_range] << [type1_id.to_i, game_type1_range_stats[type1_id][rank_range].rating.to_f.round]
							end
						end
						
						gtrr_same_rank_data = rank_rates.keys.sort.map do |rank_range|
							{ 
								'label' => h(rank_ranges[rank_range]),
								'color' => "rgb(#{col_rainbow(rank_range, 270, -30).join(", ")})",
								'data'  => rank_rates[rank_range]
							}
						end
						
						gtrr_same_rank_json = gtrr_same_rank_data.to_json
						
						gtrr_same_rank_options = {
							'legend' => {
								'position' => 'ne'
							},
							'yaxis' => {
							...
							'xaxis' => {
								'min' => 0,
								'max' => type1.length + 3,
								'ticks' => type1.to_a.map { |t| [t[0], t[1].split(//u)[0]] }
							}
						}.to_json
