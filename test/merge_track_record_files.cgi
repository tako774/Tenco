...
begin
	...
	game_ids.each do |game_id|
		...
		trn_data.sort! { |a_line, b_line| a_line[0..9].to_i <=> b_line[0..9].to_i }
		...
		File.open(data_file, 'rb') do |r|
			while (line = r.gets) do
				while (trn_data.length != 0 && trn_data[0][0..9].to_i < line[0..9].to_i) do
					merged_data << trn_data.shift
				end
				merged_data << line
				existing_data_counts += 1
			end
		end
		merged_data.concat trn_data

		res_body << "既存データ数 #{existing_data_counts} 件とマージしました\n" if DEBUG		
		res_body << "merge finished...(#{Time.now - now}/#{Process.times.utime}/#{Process.times.stime})\n" if DEBUG		
		
		# 重複データ排除
		merged_data.uniq!
		
		res_body << "重複削除対象 #{trn_data_counts + existing_data_counts - merged_data.length} 件\n" if DEBUG
		res_body << "マージ後件数 #{merged_data.length} 件...(#{Time.now - now}/#{Process.times.utime}/#{Process.times.stime})\n" if DEBUG
				
		# ファイル書き出し・移動
		File.open(data_temp_file, 'wb') do |w|
			w.print merged_data.join
		end
