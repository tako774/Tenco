...
### Pov1-game_type1:ゲーム別キャラ別レートランキングの達成記録の保存 ###
begin
...
begin
	res_body << "pov1-game_type1:ゲーム別キャラ別レートランキング処理開始\n"

	# 設定
	pov_id = 1
	pov_eval_unit_seg = SEG_V[:pov_eval_unit][:game_type1][:value].to_i
	...
	game_povs.each do |gp|
		...
		res = db.exec(<<-"SQL")
			...
			WHERE
			  ...
			  AND r.type1_id IN (#{prizes.keys.inject { |i,j| "#{i}, #{j}" } })
			ORDER BY
		...
		res = db.exec(<<-"SQL")
			...
			FROM 
			  prize_accounts pa
			WHERE
			  pa.prize_id IN (#{(prize_ids.map { |p| p }).join(",")})
		SQL
		
		res.each do |r|
			prize_accounts[r[0]] ||= {}
			prize_accounts[r[0]][r[1]] ||= {}
			prize_accounts[r[0]][r[1]][r[2]] = r[3]
		end
		...
		begin
			...
			game_account_ratings.each do |gar|
				...
				if !(prize_accounts[prizes[gar.type1_id.to_i].id] &&
				      prize_accounts[prizes[gar.type1_id.to_i].id][gar.account_id] &&
					  prize_accounts[prizes[gar.type1_id.to_i].id][gar.account_id][gar.type1_id]) then
					db.exec(<<-"SQL")
					  INSERT INTO
						prize_accounts
						(
					  ...
					  WHERE
						NOT EXISTS (
						  ...
						  FROM
							prize_accounts
						  WHERE
							prize_id = #{prizes[gar.type1_id.to_i].id.to_i}
				...
				# クラスが変わっていれば更新
				elsif prize_accounts[prizes[gar.type1_id.to_i].id][gar.account_id][gar.type1_id].to_i != game_pov_class_id.to_i then
					res_update = db.exec(<<-"SQL")
					  EXECUTE update_prize_accounts(#{game_pov_class_id.to_i}, #{prizes[gar.type1_id.to_i].id.to_i}, #{gar.account_id}, #{gar.type1_id})
					SQL
					
					if res_update.num_tuples != 1 then
						raise "PrizeAccount テーブルのアップデート対象がありませんでした：#{prizes[gar.type1_id.to_i].id.to_i}, #{gar.account_id}, #{gar.type1_id}"
					else
						updated_count += 1
					end
